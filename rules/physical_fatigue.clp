...
=>
    ...
    (if (and (or ?parkinson ?young_pci_autism) (neq ?altered_nutrition nil) ?altered_nutrition) then (bind ?physical_fatigue (+ ?physical_fatigue 1)))
    (if (and (or ?parkinson ?baseline_nutrition) (neq ?baseline_nutrition nil) ?baseline_nutrition) then (bind ?physical_fatigue (+ ?physical_fatigue 1)))
    (if (and ?parkinson (neq ?ANXIETY nil) ?ANXIETY) then (bind ?physical_fatigue (+ ?physical_fatigue 1)))
    (if (and ?parkinson (neq ?architectural_barriers nil) ?architectural_barriers) then (bind ?physical_fatigue (+ ?physical_fatigue 1)))
    (if (and (or ?parkinson ?psychiatric_patients) (neq ?water_balance nil) (< ?water_balance 1)) then (bind ?physical_fatigue (+ ?physical_fatigue 1)))
    (if (and (or ?parkinson ?psychiatric_patients ?multiple_sclerosis ?young_pci_autism) (neq ?sleep_duration_quality nil) (< ?sleep_duration_quality 6)) then (bind ?physical_fatigue (+ ?physical_fatigue 1)))
    (if (and (or ?parkinson ?multiple_sclerosis ?young_pci_autism) (neq ?heart_rate nil) (>= ?heart_rate 100)) then (bind ?physical_fatigue (+ ?physical_fatigue 1)))
    (if (and (or ?psychiatric_patients ?young_pci_autism) (neq ?baseline_heart_rate nil) (>= ?baseline_heart_rate 100)) then (bind ?physical_fatigue (+ ?physical_fatigue 1)))
    (if (and (or ?psychiatric_patients ?young_pci_autism ?parkinson ?multiple_sclerosis) (neq ?heart_rate_differential nil) (>= ?heart_rate_differential 50)) then (bind ?physical_fatigue (+ ?physical_fatigue 1)))
    (if (and (or ?older_adults ?parkinson ?psychiatric_patients ?multiple_sclerosis ?young_pci_autism) (neq ?noise_pollution nil) (> ?noise_pollution 45)) then (bind ?physical_fatigue (+ ?physical_fatigue 1)))
    (if (and (or ?older_adults ?parkinson ?psychiatric_patients ?multiple_sclerosis ?young_pci_autism) (neq ?user_reported_noise_pollution nil) (> ?user_reported_noise_pollution 45)) then (bind ?physical_fatigue (+ ?physical_fatigue 1)))
    (if (and ?older_adults (neq ?air_pollution nil) (> ?air_pollution 5)) then (bind ?physical_fatigue (+ ?physical_fatigue 1)))
    (if (and ?multiple_sclerosis (neq ?path_slope nil) ?path_slope) then (bind ?physical_fatigue (+ ?physical_fatigue 1)))
    (if (and ?older_adults (neq ?rough_path nil) ?rough_path) then (bind ?physical_fatigue (+ ?physical_fatigue 1)))
    (if (and ?parkinson (neq ?baseline_blood_pressure nil) (>= ?baseline_blood_pressure 100)) then (bind ?physical_fatigue (+ ?physical_fatigue 1)))
    (if (and ?parkinson (neq ?low_blood_pressure nil) (< ?low_blood_pressure 90)) then (bind ?physical_fatigue (+ ?physical_fatigue 1)))
    (if (and (or ?parkinson ?psychiatric_patients ?multiple_sclerosis ?young_pci_autism) (neq ?ambient_temperature nil) (> ?ambient_temperature 27)) then (bind ?physical_fatigue (+ ?physical_fatigue 1)))
