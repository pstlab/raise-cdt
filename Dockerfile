...
RUN git clone --recursive https://github.com/pstlab/raise-cdt \
    ...
    && cmake -DMONGODB_HOST=${MONGODB_HOST} -DMONGODB_PORT=${MONGODB_PORT} -DMONGODB_AUTH -DMONGODB_USER=${MONGODB_USER} -DMONGODB_PASSWORD=${MONGODB_PASSWORD} -DPOSTGRES_HOST=${POSTGRES_HOST} -DPOSTGRES_PORT=${POSTGRES_PORT} -DPOSTGRES_USER=${POSTGRES_USER} -DPOSTGRES_PASSWORD=${POSTGRES_PASSWORD} -DMQTT_HOST=${MQTT_HOST} -DMQTT_PORT=${MQTT_PORT} -DMQTT_AUTH -DMQTT_USER=${MQTT_USER} -DMQTT_PASSWORD=${MQTT_PASSWORD} -DCLIENT_DIR=${CLIENT_DIR} -DCMAKE_BUILD_TYPE=Release .. \
    && make -j$(nproc)
