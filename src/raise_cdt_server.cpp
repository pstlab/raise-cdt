    ...
    raise_cdt_server::raise_cdt_server(coco::coco_server &srv, raise_cdt &cdt) noexcept : server_module(srv), cdt(cdt)
    {
        ...
        add_path("/users/{keycloak_id}", {"get",
                                          {{"summary", "Get User"},
                                           ...
                                           {"parameters", {{{"name", "keycloak_id"}, {"in", "path"}, {"required", true}, {"schema", {{"type", "string"}}}, {"description", "The Keycloak ID of the user to retrieve."}}}},
                                           {"responses",
                                            {{"200",
        ...
        add_path("/users", {"post",
                            {{"summary", "Create User"},
                             ...
                             {"requestBody",
                              {{"required", true},
                               {"content", {{"application/json", {{"schema", {{"type", "object"}, {"properties", {{"keycloak_id", {{"type", "string"}, {"description", "The Keycloak ID for the new user."}}}}}, {"required", {"keycloak_id"}}}}}}}}}},
                             {"responses",
                              {{"201",
