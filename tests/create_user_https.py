...
if __name__ == '__main__':
    ...
    response = session.post(url + '/raise-users', headers={'Authorization': 'Bearer ' + token},
                            json={'google_id': google_id}, verify=False)
    if response.status_code != 201:
        logger.error('Failed to create user')
        sys.exit(1)
    user_uid = response.text
    logger.info(f'Created user {first_name} with google_id {google_id} and uid {user_uid}')
